WITH txn_stats AS (
  SELECT
    ...
    -- Calculate tenure as exact months since signup (no +1)
    TIMESTAMPDIFF(MONTH, u.date_joined, CURDATE()) AS tenure_months,
    COUNT(s.id) AS total_transactions,
    -- Handle NULL for users with no transactions
...
SELECT
  ...
  tenure_months,
  total_transactions,
  -- Ensure CLV = 0 for users with no transactions or tenure = 0
  ROUND(
    CASE 
      WHEN tenure_months = 0 OR total_transactions = 0 THEN 0
      ELSE 
        (total_transactions / tenure_months) * 12 
        * ((avg_txn_amount_kobo / 100.0) * 0.001)
    END,
    2
  ) AS estimated_clv
FROM
